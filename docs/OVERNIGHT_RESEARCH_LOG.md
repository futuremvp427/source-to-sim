...
- Polymarket's current fee documentation is important for any translated strategy: Weather taker fee rate is 0.05 in the documented `C * feeRate * p * (1-p)` formula; makers are not charged, and Weather is eligible for maker rebates funded from taker fees. Thus a future maker-style strategy must model adverse selection separately from taker execution, not simply subtract the taker curve from every fill: https://docs.polymarket.com/trading/fees and https://docs.polymarket.com/market-makers/maker-rebates
