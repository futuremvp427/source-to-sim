...
const source = readFileSync(new URL("./sports-shadow-settlement.ts", import.meta.url), "utf8");

describe("sports shadow SETTLEMENT job scheduler hook authorization contract", () => {
  ...
  it("CODEX P2-1: invokes the independent runSportsShadowSettlementJob, never runSportsShadowCycle", () => {
    expect(source).toContain("runSportsShadowSettlementJob()");
    expect(source).not.toContain("runSportsShadowCycle");
  });
});
