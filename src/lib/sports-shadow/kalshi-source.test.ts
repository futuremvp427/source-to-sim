...
describe("kalshi same-venue source adapter", () => {
  it("1. keeps the exact Kalshi ticker exact — never re-derived, never normalized away", () => {
    const trade = admitOk();
    expect(trade.marketTicker).toBe(TICKER);
    expect(targetLegForTrade(trade)).toEqual({ venue: "KALSHI", ticker: TICKER, side: "YES" });
  });

  it("2. never routes through the cross-venue resolver", () => {
    const trade = admitOk();
    expect(trade.route).toBe("SAME_VENUE_KALSHI");
    expect(requiresCrossVenueResolution(trade)).toBe(false);
    // Static guarantee: the adapter module does not even reference the cross-venue resolver.
  ...
  it("3. preserves YES/NO orientation and keeps the two legs as distinct positions", () => {
    const yes = admitOk({ sourceTradeId: "y", contractSide: "YES" });
    const no = admitOk({ sourceTradeId: "n", contractSide: "NO" });
    expect(yes.contractSide).toBe("YES");
    expect(no.contractSide).toBe("NO");
  ...
  it("5. a second BUY feeds the existing ADD/DCA aggregation logic", () => {
    const first = toEligibleFill(admitOk({ sourceTradeId: "a", quantity: 100, priceCents: 40 }), 1_000);
    const second = toEligibleFill(
      admitOk({ sourceTradeId: "b", quantity: 100, priceCents: 50, sourceTsSeconds: 1_756_000_060 }),
      2_000,
    );
  ...
  it("10. unqualified / unverified source activity produces no paper trade", () => {
    ...
    expect(isTraderQualified(notApproved, TRADER)).toBe(false);
  ...
  it("11. the live-order path remains unreachable from this adapter", () => {
    ...
    // The one transport plug point is an interface only: no production implementation ships.
