...
### WEATHER-HYBRID-2 Short-horizon repricing test — BeefSlayer trigger REJECTED

- Status: BOUNDED TEST COMPLETE. VERDICT: REJECTED. PAPER/RESEARCH ONLY.
- Tests a narrower, different hypothesis than direct-copy (already REJECTED separately): not "hold the same bucket to U.S. settlement" but "does a BeefSlayer trade predict short-horizon U.S. Kalshi price movement, such that a fast follower could scalp it." Motivated by reverse-engineering BeefSlayer's actual on-chain lifecycle (buys and sells the same position 15-45 minutes later on the source venue; see WEATHER-DECISION-1).
- Script `scripts/research-weather-hybrid-repricing.mjs`; workflow `.github/workflows/weather-hybrid-repricing.yml`; CI run `33017995163`, success, SHA `936ee05`.
- Window 2026-04-28..2026-08-25 (120d). 186 first-per-city/date/bucket signals. Lane A (exact bucket): 40 priced / 25 station-days. Lane B (event-level, probability-matched, no lookahead): 185 priced / **90 station-days**. Control (matched non-trigger, same city/hour/price-band): 169 points / 122 station-days.
- **KEY TEST (+5m detection delay, +15m hold, +1c adverse entry, Lane B): N=85 station-days, P/L -$682.94, ROI -18.9%, PF 0.02x, win rate 4.7%.** Bootstrap 95% CI on total P/L: -$845.57 to -$546.97 (does not cross zero). After removing top 1%/5% winning days: -$690.79 (barely changes — not outlier-dependent).
- Negative at every delay (0/1/5/15m) x hold (1/5/15/30/45m) x adverse (0/1/2/3c) combination tested, Lane A and Lane B alike. PF never exceeds 0.33x anywhere in the full grid, against the required 1.3x. Negative even at BASE (zero adverse stress): key cell P/L -$512.10.
- Every predefined segmentation cell is negative: by source price band, city, time of day, size band, first-buy-vs-DCA, single-vs-multi-bucket, later-sold-vs-held. No promising sub-population exists.
- **Control comparison: BeefSlayer trigger (N=85, ROI -18.9%, PF 0.02x) is statistically indistinguishable from matched non-trigger control (N=110, ROI -19.3%, PF 0.03x).** No evidence the wallet's trigger adds predictive value beyond ordinary market movement; the deep negative result in both groups reflects the structural cost of paying the spread and fees on both legs of a round trip, not a wallet-specific effect.
- Per the pre-registered stop rule (negative after +1c adverse, PF <1.3, or outlier-dependent => REJECTED): **REJECTED**, decisively and on a well-powered sample (90 station-days, above the 50 minimum, near the 100 preferred threshold).
- B/C (Weather/NBM signal, market baseline) comparison: B and C marked `DATA_UNAVAILABLE` for this run as instructed — out of scope for this bounded task, not fabricated. A no-lookahead historical station-observation source for a future attempt was identified and verified separately (IEM ASOS archive, mesonet.agron.iastate.edu, official ASOS/METAR-derived, confirmed station-identity match and 2026 coverage for all five audited cities) — do not substitute reanalysis for it.
- **Two real bugs found while smoke-testing and fixed in all three hybrid scripts** (`research-weather-hybrid-preflight.mjs`, `research-weather-hybrid-trigger-funnel.mjs`, and the new repricing script): (1) `fetchJson`'s non-retryable-status throw was inside its own try/catch, so a 404 was caught and retried through the full exponential backoff (~25s) instead of failing fast; (2) `candleWindow` had dropped the `/series/{series}/markets/{ticker}/candlesticks` fallback that the already-committed direct-copy script correctly used — the primary `/historical/markets/{ticker}/candlesticks` route silently returns zero candles for many real, heavily-traded tickers (confirmed on a $47k-volume market). Verified no regression: rerunning the direct-copy script after the fetchJson fix reproduces its exact prior numbers (BeefSlayer 38/24, ColdMath 7/7), CI run `33017995137` success.
- **Separately, an unrelated pre-existing CI regression was found and is NOT fixed here (out of scope):** Audit CI's `schema-contract` job fails reproducibly (twice, non-flaky) on `weather-us-translation-research` at commit `f65db22` on `sports_shadow_*` table grants (anon/authenticated SELECT allowed where the contract expects it denied), despite that commit touching no SQL/migration files and passing on the immediately-prior commit `7b3bcec`. Most likely CI runner/image drift, not a weather-lab-caused regression. Sports Shadow is explicitly out of scope for this task; flagged for separate attention.
- No production deployment, no Sports Shadow change, no Supabase modification, no Lovable credits, no orders, `LIVE_EXECUTION_IMPLEMENTED=false` unchanged, `main` untouched.

### WEATHER-HYBRID-3 Wallet-free NBM revision / observation crossing shootout — ALL REJECTED

- Status: BOUNDED SHOOTOUT COMPLETE. VERDICT: ALL REJECTED/DATA_INSUFFICIENT. WEATHER DIRECTIONAL EDGE NOT DEMONSTRATED. PAPER/RESEARCH ONLY.
- BeefSlayer-as-trigger stays REJECTED (WEATHER-HYBRID-2) and was not retested. This asks a wallet-free question: does the weather data itself (NBM cycle revisions, real station-observation threshold crossings, or their combination) predict Kalshi price movement.
- Script `scripts/research-weather-signal-shootout.mjs`; workflow `.github/workflows/weather-signal-shootout.yml`; CI run `33021361373`, success, SHA `c009c77`.
- Window 2026-05-18..2026-08-25 (100d), 5 cities, settlement-fingerprinted per market from its own rules_primary text (NWS vs The Weather Company); any station-day with an unknown/mixed-provider leg excluded before signal detection.
- **A NBM_CYCLE_REVISION_EDGE** (consecutive NBM station-text cycles at preregistered 12/15/18/21Z disagree by >=10 probability points on the previously-modal bucket, threshold fixed before any run): 164 signals / **148 station-days**. KEY LINE (+5m delay/+15m hold/+1c adverse): N=117, P/L -$450.36, ROI -9.7%, PF 0.11x, win rate 8.5%. Bootstrap 95% CI -$559.78..-$342.70 (does not cross zero). Negative across the full delay(0/1/5/15m) x hold(5/15/30/45/60m) grid, PF never exceeding 0.59x. Negative in every one of the 5 cities individually. **REJECTED.**
- **B OBSERVATION_THRESHOLD_CROSSING_EDGE** (purely mechanical: which bucket's own bounds contain the running local-day max from real IEM ASOS observations, official ASOS/METAR archive, not reanalysis; flag when that bucket changes): 859 signals / **420 station-days**. KEY LINE: N=417, P/L -$4,350.23, ROI -16.0%, PF 0.12x, win rate 15.3%. Bootstrap 95% CI -$5,005.00..-$3,764.57. Negative across the full grid (PF never exceeding 0.46x) and in every city. **REJECTED.**
- **C COMBINED** (simple preregistered AND: an A revision and a B crossing on the same bucket within 60 minutes, no scoring formula): only 13 signals / 12 station-days -- below the 50-station-day minimum. **DATA_INSUFFICIENT**, not forced to a verdict either way.
- **D MARKET_ONLY_CONTROL** (matched non-signal station-days, same city/hour/price-band construction as the repricing script's control): N=23, P/L -$55.79, ROI -5.8%, PF 0.15x, bootstrap 95% CI -$87.79..-$22.74 -- also negative (structural round-trip spread+fee cost), but **materially smaller in magnitude than A (-9.7%) or B (-16.0%)**. A and B are not merely indistinguishable from the market-only baseline the way BeefSlayer's trigger was -- they perform **worse** than it on this sample.
- One real bug found and fixed while smoke-testing (before the full run, not after seeing results): the observation-crossing "modal bucket" heuristic computed a Normal-distribution mean via `Math.max(runningMax, ...bucket lower bounds)`, letting a bucket's own fixed lower bound dominate regardless of the actual observed temperature, so the modal bucket almost never changed and 0 crossings were found in an 8-day smoke test. Replaced with the mechanical containment rule the task itself specifies. Verified: 0 -> 63 crossing signals after the fix, same 8-day window.
- **Secondary finding, does not change the verdict:** A's HOLD-TO-SETTLEMENT lane (kept separate from short-horizon per the task's instruction) showed nominally positive economics at +5m delay/+1c adverse -- N=127, P/L +$344.91, ROI +7.0%, PF 1.26x -- but does not survive scrutiny: independently computed bootstrap 95% CI is **-$460.69..+$1,166.58 (crosses zero)**, and P/L drops from $344.91 to $101.65 (a ~70% reduction) after removing just the top 5% of winning station-days. PF also sits just under the 1.3 gate. Interesting enough to record, not strong enough to promote.
- Calibration (A): Brier 0.3455, log loss 0.9958 on N=164 -- poorly calibrated (e.g. the 60-80% forecast-probability bin had only a 28.0% actual hit rate, a large overconfidence gap). This uses a deliberately simple Normal-CDF probability model (NBM peak forecast + TSD only, no intraday conditioning, no basis-mismatch guard) for this bounded script, NOT the full Weather Lab probability engine on the sibling branch -- a real scoping limitation, not a claim that a more sophisticated model would also fail.
- TSD-vs-GEFS-ensemble sigma comparison: only N=7 comparable pairs (Open-Meteo's ensemble archive did not cover most of the 100-day window) -- **DATA_INSUFFICIENT for this sub-question**, not a real finding; do not cite "ensemble beat TSD" from this sample.
- Capacity: moot, nothing survived the gate. Live spot check for reference: ~$1,768 resting depth within 5c of best on a single Chicago bucket -- consistent with the modest, low-hundreds-to-low-thousands-of-dollars-per-opportunity capacity already established throughout this project.
- **Per the pre-registered stop rule and pass gate: WEATHER DIRECTIONAL EDGE NOT DEMONSTRATED under this design. Recommend stopping this strategy family rather than inventing another variant.**
- The pre-existing, reproducible Sports Shadow `schema-contract` CI regression on `weather-us-translation-research` (recorded in WEATHER-HYBRID-2) remains unfixed and out of scope; not touched in this task.
- No production deployment, no Sports Shadow change, no Supabase modification, no Lovable credits, no orders, `LIVE_EXECUTION_IMPLEMENTED=false` unchanged, `main` untouched.
