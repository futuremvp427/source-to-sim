CREATE OR REPLACE FUNCTION public.get_pending_experiment_source_events(p_experiment_id uuid, p_limit integer DEFAULT 300)
 RETURNS TABLE(id uuid, event_key text, asset text, market_title text, outcome text, side text, shares numeric, price numeric, source_ts bigint, first_seen_at timestamp with time zone)
 LANGUAGE sql
...
DROP INDEX IF EXISTS public.source_events_wallet_ts_key_idx;